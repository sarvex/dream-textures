        ...
        case Pipeline.STABLE_DIFFUSION:
            ...
            class GeneratorPipeline(diffusers.StableDiffusionInpaintPipeline):
                def prepare_depth(self, depth, image, dtype, device):
                    device = torch.device(
                        'cpu' if device.type == 'mps' else device.type
                    )
                    if depth is None:
                        ...
                        depth_min = torch.amin(
                            depth_map, dim=[1, 2, 3], keepdim=True
                        )
                        depth_max = torch.amax(
                            depth_map, dim=[1, 2, 3], keepdim=True
                        )
                        depth_map = (
                            2.0
                ...
                def prepare_img2img_latents(
                    ...
                    if image is not None:
                        ...
                        if isinstance(generator, list):
                            image_latents = [
                                self.vae.encode(image[:1]).latent_dist.sample(
                                    generator[i]
                                )
                                for i in range(batch_size)
                            ]
                ...
                def get_timesteps(self, num_inference_steps, strength, device):
                    ...
                    init_timestep = (
                        int(num_inference_steps * strength) + offset
                    )
                    init_timestep = min(init_timestep, num_inference_steps)

                    t_start = max(
                        num_inference_steps - init_timestep + offset, 0
                    )
                ...
                def __call__(
                    ...
                    image: Optional[
                        Union[torch.FloatTensor, PIL.Image.Image]
                    ] = None,
                    strength: float = 0.8,
                    ...
                    **kwargs,
                    ...
                    height = (
                        height
                        or self.unet.config.sample_size * self.vae_scale_factor
                    )
                    width = (
                        width
                        or self.unet.config.sample_size * self.vae_scale_factor
                    ...
                    depth = self.prepare_depth(
                        depth_image, image, text_embeddings.dtype, device
                    )
                    ...
                    if (
                        num_channels_latents + num_channels_depth
                    ...
                    with self.progress_bar(
                        ...
                        for i, t in enumerate(timesteps):
                            ...
                            latent_model_input = torch.cat(
                                [latent_model_input, depth], dim=1
                            )
                    ...
                    yield ImageGenerationResult(
                        [
                            np.asarray(
                                PIL.ImageOps.flip(image).convert('RGBA'),
                                dtype=np.float32,
                            )
                            / 255.0
            ...
            depth_image = (
                PIL.ImageOps.flip(
                    PIL.Image.fromarray(np.uint8(depth * 255)).convert('L')
                ).resize(rounded_size)
            ...
            if seamless_axes == SeamlessAxes.AUTO:
                init_sa = (
                    ...
                    else self.detect_seamless(np.array(init_image) / 255)
                )
                depth_sa = (
                    None
                    if depth_image is None
                    else self.detect_seamless(
                        np.array(depth_image.convert('RGB')) / 255
                    )
                )
                if init_sa is not None and depth_sa is not None:
                    seamless_axes = SeamlessAxes(
                        (init_sa.x and depth_sa.x, init_sa.y and depth_sa.y)
                    )
